...
$(function(){
    ...
    getNewsByid({id}).then(res=>{
        ...
        $('.news_name').html(res.data.news.name)
        ...
        if(res.data.beforeNews){
            be = `
           <a href='news_detail.html?id=${res.data.beforeNews.id}' class="prev fl ellipsis">
        ...
        if(res.data.nextNews){
            af = `
           <a href='news_detail.html?id=${res.data.nextNews.id}' class="next fr ellipsis">
